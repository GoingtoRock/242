...
class RouteInfo:
    ...
    def shortestroute(self):
        ...
        cityList = GraphLibrary.city_list
        isValid = False
        while not isValid:
        ...
        while not isValid:
            ...
            isValid = end in cityList
            if start == "q" or start == "exit":
                return

        path = self.graph.minDistance((self.graph.findNode(start)),
                                      (self.graph.findNode(end)))
